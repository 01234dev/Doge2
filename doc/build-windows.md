...
Below are some notes on how to build Dogecoin2 Core for Windows.
...
To build Dogecoin2 from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Dogecoin2 GitHub repository](https://github.com/LivingLitecoin/Doge2/)).
...
Note that for WSL v1 the Dogecoin2 Core source path MUST be somewhere in the default mount file system, for
example /usr/src/dogecoin2, AND not under, for example, /mnt/d/dogecoin2.
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Dogecoin2's dependencies):
...
way. This will install to `c:\workspace\dogecoin2`, for example:

    make install DESTDIR=/mnt/c/workspace/dogecoin2
