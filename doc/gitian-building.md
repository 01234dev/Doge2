...
`gitian-build.sh` is a standalone script, it can be downloaded and run outside of Dogecoin2 Core repository.
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/dogecoin2-linux-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/dogecoin2-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/dogecoin2-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/dogecoin2/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [dogecoin2/gitian.sigs](https://github.com/dogecoin2/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.

**When your PR is merged, you will be recorded for all future history as a *Gitian Builder of Dogecoin2 Core*!**
