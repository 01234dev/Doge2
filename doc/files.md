...
The data directory is the default location where the Dogecoin2 Core files are stored.
...
Windows  | `%APPDATA%\Dogecoin2\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `dogecoin2.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `dogecoin2.conf`       | User-defined configuration settings for `dogecoin2d` or `dogecoin2-qt`; can be specified by `-conf` option
`./`               | `dogecoin2d.pid`        | Stores the process ID (PID) of `dogecoin2d` or `dogecoin2-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `dogecoin2d` or `dogecoin2-qt`
...
`dogecoin2-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Dogecoin2 Core:
