...
You must install required dependencies to build a basic Dogecoin2 daemon, optional dependencies may vary according to your requirements.
...
Dogecoin2 Core's released binaries are built using the "depends system", which
...
**Optional dependencies** :
```bash
# Qt (required for dogecoin2-qt GUI)
...
According to installed dependencies, the following steps will compile `dogecoin2d`, `dogecoin2-cli` and `dogecoin2-qt`.
...
#### Enable Qt GUI
Create `dogecoin2-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Dogecoin2 Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Dogecoin2 Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Dogecoin2 may be compiled in
...
#### Security
To help make your Dogecoin2 installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Dogecoin2 should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Dogecoin2 Core.
