...
#if defined(NDEBUG)
# error "Dogecoin2 cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Dogecoin2 Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetDogecoin2MinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Dogecoin2: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("dogecoin2-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Dogecoin2: BIP30 has been active since inception
    ...
    // Dogecoin2: BIP16 has been enabled since inception
    ...
    // Start enforcing BIP68 (sequence locks) and BIP112 (CHECKSEQUENCEVERIFY) at the fixed activation height.
    ...
    // Start enforcing WITNESS rules at the fixed activation height.
    ...
    CAmount blockReward = nFees + GetDogecoin2BlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
{
    // Dogecoin2: Disable SegWit
    return pindexPrev != nullptr && (pindexPrev->nHeight + 1) >= 300;
    // LOCK(cs_main);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Dogecoin2: Disallow AuxPow blocks before it is activated.
    ...
    // Dogecoin2: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Dogecoin2: We probably want to disable this
    int nLockTimeFlags = 0;
    if ((pindexPrev->nHeight + 1) >= 300) {
