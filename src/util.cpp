...
const char * const BITCOIN_CONF_FILENAME = "dogecoin2.conf";
const char * const BITCOIN_PID_FILENAME = "dogecoin2d.pid";
...
#else
    const char* pszModule = "dogecoin2";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Dogecoin2";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Dogecoin2";
#else
    // Unix
    return pathRet / ".dogecoin2";
#endif
#endif
}
