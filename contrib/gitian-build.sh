...
# Dependencies
ossPatchUrl="https://depends.dogecoin2core.org/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://depends.dogecoin2core.org/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://depends.dogecoin2core.org/MacOSX10.11.sdk.tar.gz"
macosSdkHash="bec9d089ebf2e2dd59b1a811a38ec78ebd5da18cbbcd6ab39d1e59f64ac5033f"

liefUrl="https://depends.dogecoin2core.org/lief-0.12.3-cp38-cp38-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"
...
SIGNER=
VERSION=
url=https://github.com/LivingLitecoin/Doge2
proc=2
mem=2000
...
Standalone script to perform the gitian build of Dogecoin2 Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/LivingLitecoin/Doge2
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/dogecoin2-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/dogecoin2/gitian.sigs.git
    git clone https://github.com/LivingLitecoin/Doge2-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/dogecoin2-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean dogecoin2 git directory because of old caching
    if [ -d inputs/dogecoin2/ ]; then
        echo "Cleaning Dogecoin2 directory..."
        rm -rf inputs/dogecoin2/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit dogecoin2="$COMMIT" --url dogecoin2="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
